...
// Uses admin-side charts logic for consistent data aggregation and filtering
// =======================
...
function populateYearFilter(data) {
	...
	// Preserve existing year options and add any new years from data
	const existing = new Set(Array.from(filterYearEl.options).map(o => o.value));
	
	// Add years from data that aren't already in the dropdown
	years.forEach(y => {
	...
	// Re-sort all year options (keeping 'all' at the top)
	const allOptions = Array.from(filterYearEl.options);
	const allOption = allOptions.find(o => o.value === 'all');
	const yearOptions = allOptions.filter(o => o.value !== 'all').sort((a, b) => b.value.localeCompare(a.value));
	
	// Clear and rebuild
	filterYearEl.innerHTML = '';
	if (allOption) filterYearEl.appendChild(allOption);
	yearOptions.forEach(opt => filterYearEl.appendChild(opt));
}
...
function renderFromStats(stats, reports) {
	...
	// Update the real-time summary for current (filtered) reports
	if (summaryEl) {
		try {
			...
			if (totalReported === 0) {
				...
				// Rank categories by reported count and describe the top types (up to 3)
