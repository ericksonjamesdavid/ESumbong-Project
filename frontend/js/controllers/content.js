...
function renderAnnouncementList(items) {
    ...
    Array.from(container.children).forEach(child => {
        if (child.id !== 'announcementEmptyState') child.remove();
    });
    ...
    items.forEach(item => {
        ...
        el.innerHTML = `
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 items-start"> 
                <h4 class="font-bold text-green-900 text-lg break-words w-full sm:w-auto">${item.title}</h4>
                    <div class="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                    <button class="edit-btn flex-1 sm:flex-none bg-gray-100 hover:bg-green-100 text-green-700 px-3 py-1 rounded transition text-sm font-medium">Edit</button>
                    <button class="delete-btn flex-1 sm:flex-none bg-gray-100 hover:bg-amber-100 text-amber-600 px-3 py-1 rounded transition text-sm font-medium">Archive</button>
                </div>
            </div>
...
function renderNewsList(items) {
    ...
    // Clear all items but keep empty state
    Array.from(container.children).forEach(child => {
        if (child.id !== 'newsEmptyState') child.remove();
    });
    ...
    items.forEach(item => {
        const el = document.createElement('div');
        el.className = "bg-white p-4 rounded-lg border border-gray-200 hover:shadow-md transition flex flex-col sm:flex-row gap-4";
        const imageUrl = item.imageUrl && !item.imageUrl.startsWith('http') ? '/' + item.imageUrl : item.imageUrl;
        el.innerHTML = `
            <img src="${imageUrl}" class="w-full h-48 sm:w-24 sm:h-24 object-cover rounded-md bg-gray-100 flex-shrink-0">
            <div class="flex-grow min-w-0">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                    <div class="min-w-0 flex-grow">
                        <h4 class="font-bold text-emerald-900 text-lg leading-tight break-words">
                    ...
                    <div class="flex gap-2 w-full sm:w-auto flex-shrink-0">
                        <button class="edit-btn flex-1 sm:flex-none bg-gray-100 hover:bg-emerald-100 text-emerald-700 px-3 py-1 rounded transition text-sm font-medium">Edit</button>
                        <button class="delete-btn flex-1 sm:flex-none bg-gray-100 hover:bg-amber-100 text-amber-600 px-3 py-1 rounded transition text-sm font-medium">Archive</button>
                    </div>
                </div>
...
function renderArchiveCard(item, type) {
    ...
    el.innerHTML = `
        <div class="flex items-start gap-4">
            ${type === 'news' && imageUrl ? 
                `<img src="${imageUrl}" class="w-12 h-12 rounded object-cover grayscale opacity-60">` : ''}
